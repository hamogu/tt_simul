...
; dens=[9.,9.5,10.,10.5,11.,11.5,12.,12.5,13.]
; makechiantitable,temp,dens,'chianti2.fits'


pro makechiantitable,intpar1,intpar2,fitsfile,debug=debug
...
maxn_intval=n_elements(intpar1)>n_elements(intpar2)

parameters=replicate({NAME:'Temp          ',METHOD:long(0),INITIAL:1.,DELTA:.1,MINIMUM:min(intpar1),BOTTOM:min(intpar1),TOP:max(intpar1),MAXIMUM:max(intpar1),NUMBVALS:long(maxn_intval),VALUE:findgen(maxn_intval)},naddparm+nintparm)
parameters[0].numbvals=long(n_elements(intpar1))
parameters[0].value=intpar1
parameters[1].name='log(n_e)'
parameters[1].initial=median(intpar2)
parameters[1].delta=abs(intpar2[3]-intpar2[2])
parameters[1].minimum=min(intpar2)
parameters[1].bottom=min(intpar2)
parameters[1].top=max(intpar2)
parameters[1].maximum=max(intpar2)
parameters[1].numbvals=long(n_elements(intpar2))
parameters[1].value=intpar2
; elements
...
for ipar1=0,n_elements(intpar1)-1 do begin
 for jpar2=0,n_elements(intpar2)-1 do begin
  spectra[ipar1*n_elements(intpar2)+jpar2].paramval=[intpar1[ipar1],intpar2[jpar2]]
  tt_synthetic, min_lambda,max_lambda,logem=0,logt_i=alog10(intpar1[ipar1]/8.617e-8), /phot,rphot=1,radtem=6e3,output=lines, master=sngl_ion.master,dens=10.^intpar2[jpar2],ioneq_n='/usr/local/hssoft/chianti/dbase/ioneq/mazzotta_etal_ext.ioneq'
  make_chianti_spec, lines,reverse(lambda_scale),spectrum,/cont,/phot,abund='temp_master.abund'
  spectra[ipar1*n_elements(intpar2)+jpar2].intpspec=factor*reverse(spectrum.spectrum)
  debug[0,*]=reverse(spectrum.continuum)
  for k=0,naddparm-1 do begin
    tt_synthetic, min_lambda,max_lambda,logem=0,logt_i=alog10(intpar1[ipar1]/8.617e-8), /phot,rphot=1,radtem=6e3,output=lines,sngl=sngl_ion.(k+1),dens=10.^intpar2[jpar2],ioneq_n='/usr/local/hssoft/chianti/dbase/ioneq/mazzotta_etal_ext.ioneq'
    read_abund,base_abund,abund,abund_ref
    ...
    spectra[ipar1*n_elements(intpar2)+jpar2].(k+2)=factor*reverse(spectrum.spectrum)*1e-12
  endfor
 endfor
endfor
