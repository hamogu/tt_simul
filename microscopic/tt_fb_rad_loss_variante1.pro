...
;-
pro tt_fb_rad_loss, recomb,recomben,t_ion,noverner=noverner
...
IF NOT keyword_set(noverner) THEN BEGIN
  ...
  free_lun,lun
ENDIF
FOR iz=1,n_elements(recomb[*,0]) DO BEGIN		;n_elements(recomb[*,0]) = number of chemical elements
    ...
    FOR ion=1,iz DO BEGIN
      ...
      IF total(ieq) NE 0. THEN BEGIN
        ...
        recomben[iz-1,ion]=total(ab*(ident_wvl#ieq)*1.e-40*4*!pi)		;4*pi = emission in all directions
