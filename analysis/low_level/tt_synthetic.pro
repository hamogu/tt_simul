...
PRO tt_synthetic, wmin, wmax, output=output, err_msg=err_msg, msg=msg, $
      ...
      goft=goft, ioneq_name=ioneq_name, dem_name=dem_name, $
      noprot=noprot, rphot=rphot, radtemp=radtemp, progress=progress, $
      no_sum_int=no_sum_int,  group=group, tt_simul=tt_simul
...
IF n_elements(logt_isothermal)  NE 0 THEN BEGIN
   ...
   if ~keyword_set(tt_simul) then begin
...
;ask for at least three points:
	 ...
	 if keyword_set(tt_simul) then dem_int1=10.^ioneq_logt[gdt] else dem_int1=10.^(SPLINE(dem_logt,dem,ioneq_logt[gdt]))
...
;      minimum and maximum Te values given as input.
      ...
      ;following line is necessary because there could be equal temperatures, t_index may contain indices which are not included in ind_gioneq because there ioneq=0
      IF keyword_set(tt_simul) then t_index=t_index[where(t_index ge min(ind_gioneq) and t_index le max(ind_gioneq))]
...
; -----------------------------------------
         ...
         IF status lt 0 THEN ionrec = {rec:0., ci:0., temp:0., lev_up_rec:0., lev_up_ci:0., status:-1, ioneq:0.}
         IF status gt 0 THEN BEGIN
              ...
              lev_up_ci:lupci, status:status, ioneq:ioneq_ionrec}
         ENDIF
...
;do a spline interpolation in the logs:
            if keyword_set(tt_simul) then begin
	      ion_frac=this_ioneq[ind_gioneq]
	    endif else begin  
...
;-------------------------------------------------------------
         ...
         IF model_name EQ  'Function' THEN BEGIN 
            IF (model_file NE ' ' and ~keyword_set(tt_simul)) THEN $
              DENS = 10.^SPLINE(alog10(temperature),alog10(density), alog10(TEMP),/double ) ELSE $
...
; Calculates intensities or contribution functions, either isothermal or not
                     ...
                     IF n_elements(logt_isothermal) EQ 0 THEN BEGIN   ; Non-isothermal case
                        ...
                        IF keyword_set(goft) THEN BEGIN    ; Contribution functions
                           ...
                           this_goft = fltarr(n_ioneq_logt,nn)
                     ...
                     ENDIF  ELSE BEGIN                                ; Isothermal case
                        ...
                        IF keyword_set(no_sum_int) THEN BEGIN
                           ...
                           t_temp = 0.*intensity
                           FOR i=0,nt-1 DO BEGIN
                               temp_intensity=transpose(de_nj_Aji[i,*])#ieq_dem_dt[i]
                               intensity[i,*]=temp_intensity[*]
                               t_temp[i,*]=logt_isothermal[i]
                           ENDFOR
...
;flag any lines with theoretical wavelength with a -1
                     ...
                     IF NOT keyword_set(goft) THEN BEGIN
                        IF keyword_set(no_sum_int) AND n_elements(logt_isothermal) NE 0 THEN BEGIN
                           ...
                           ENDIF ELSE BEGIN                              ; If they do not overlap, stores only those who do
                               ...
                               ENDFOR
                               FOR i=t_index(0),t_index(nt-1) DO BEGIN
                                   list_int_new[i,nn_exist:nn_new-1]=t_list_int[i-t_index(0),*]  ; Stores the new ones
                               ENDFOR
                               list_int=list_int_new
                           END
                        ENDIF ELSE BEGIN
                     ...
                     ENDIF ELSE BEGIN
                           ...
                           nn_new=nn+nn_exist
                           list_goft_new=fltarr(n_ioneq_logt,nn_new)
